using UnityEngine;
//Accion que permite mostrar un icono (un corazon por ejemplo)
//que sea un objeto hijo del objeto agent
public class ShowIcon : Action {

    GameObject agent;
    string name;//nombre del componente del icono

    public ShowIcon(GameObject Agent, string Name){
        agent = Agent;
        name = Name;
    }
        ...
        for(int i  = 0; i < childs.Length; i++){//para cada hijo
            if(childs[i].name == name){//si tiene el nombre que buscamos
                ...
                if(!(sprite is null)){//si tiene sprite
                    sprite.enabled = true;//lo mostramos
